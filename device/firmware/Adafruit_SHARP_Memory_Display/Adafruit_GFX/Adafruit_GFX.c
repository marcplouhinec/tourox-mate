...
// Private variables
static Adafruit_GFX_draw_pixel_t m_p_draw_pixel;
static int16_t _rawWidth, _rawHeight; // Display w/h as modified by current rotation
...
static void Adafruit_GFX_drawPixel(int16_t x, int16_t y, uint16_t color) {
	m_p_draw_pixel(x, y, color);
}

void Adafruit_GFX_init(int16_t w, int16_t h, Adafruit_GFX_draw_pixel_t p_draw_pixel) {
	m_p_draw_pixel = p_draw_pixel;
...
size_t Adafruit_GFX_writeChar(uint8_t c)
{
    if (c == '\n') {
        cursor_y += textsize*8;
        cursor_x  = 0;
...
void Adafruit_GFX_writeText(char* text) {
	...
	for (int16_t i = 0; i < textLen; i++) {
		Adafruit_GFX_writeChar(text[i]);
	}
}
